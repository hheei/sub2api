...
retry_count, version, session_id, output_expires_at, is_dynamic_rate, input_deleted_at, output_deleted_at, downloaded_at, user_deleted_at,
...
func scanBatchImageJob(row rowScanner) (*service.BatchImageJob, error) {
	...
	err := row.Scan(
		...
		&job.RetryCount, &job.Version, &sessionID, &outputExpiresAt, &job.IsDynamicRate, &inputDeletedAt, &outputDeletedAt, &downloadedAt, &userDeletedAt,
